                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            <label for="description" class="form-label d-flex justify-content-between align-items-center">
                                ...
                                <span>
                                    <button type="button" class="btn btn-sm btn-primary rounded-pill px-2" id="geminiGenDescBtnEdit" style="font-size:11px;"><i class="fe-zap"></i> ✨ Gemini AI Regenerate</button>
                                    <select id="geminiDescLangEdit" style="font-size:11px; padding:2px 6px; border-radius:12px; border:1px solid #dee2e6;"><option value="bn">বাংলা</option><option value="en">English</option></select>
                                </span>
                            </label>
                            <textarea name="description" rows="6" id="productDescriptionEdit"
                            ...
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                            <small class="text-muted">Gemini AI দিয়ে description পুনরায় লিখুন।</small>
                        </div>
                        ...
                        <div id="variant-wrapper">
                            ...
                            @forelse($groupedByColor as $colorId => $variantsForColor)
                                ...
                                <div class="variant-card variant-item">
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="variant-size-stocks mt-1" @if(empty($sizeIds)) style="display:none;" @endif>
                                                <label class="form-label fw-bold mb-1"><i class="fe-box me-1"></i> Size-wise Stock &amp; Price</label>
                                                <div class="size-stock-rows row g-2">
                                                    @foreach($variantsForColor as $sv)
                                                        @if($sv->size_id)
                                                        <div class="col-md-3 col-6 size-stock-item" data-size-id="{{ $sv->size_id }}">
                                                            <div class="border rounded p-2 bg-white">
                                                                <div class="fw-bold mb-1" style="font-size:12px;">Size: {{ optional($sv->size)->sizeName ?? 'N/A' }}</div>
                                                                <input type="number" min="0" class="form-control form-control-sm size-stock-input mb-1" placeholder="Stock" value="{{ $sv->stock }}">
                                                                <input type="number" step="0.01" min="0" class="form-control form-control-sm size-price-input" placeholder="Price (optional)" value="{{ $sv->price }}">
                                                            </div>
                                                        </div>
                                                        @endif
                                                    @endforeach
                                                </div>
                                                <small class="text-muted d-block mt-1"><i class="fa fa-info-circle"></i> প্রতিটি সাইজের আলাদা স্টক ও দাম দিন। খালি রাখলে Default Price/Stock ব্যবহার হবে।</small>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.addEventListener('click', function (e) {
        ...
        if (target.classList.contains('add-variant')) {
            ...
            newRow.find('.variant-size-stocks').hide().find('.size-stock-rows').empty();
    ...
    // ===== Size-wise Stock & Price rows (edit) =====
    function buildSizeStockRows($row) {
        ...
        $rows.find('.size-stock-item').each(function () {
            var sid = String($(this).data('size-id'));
            existing[sid] = {
        ...
        selected.forEach(function (sizeId) {
            ...
            var prev = existing[String(sizeId)] || {};
            var html =
                '<div class="col-md-3 col-6 size-stock-item" data-size-id="' + sizeId + '">' +
                    '<div class="border rounded p-2 bg-white">' +
                        ...
                        '<input type="number" min="0" class="form-control form-control-sm size-stock-input mb-1" placeholder="Stock" value="' + (prev.stock !== undefined ? prev.stock : '') + '">' +
                        '<input type="number" step="0.01" min="0" class="form-control form-control-sm size-price-input" placeholder="Price (optional)" value="' + (prev.price !== undefined ? prev.price : '') + '">' +
                    '</div>' +
                '</div>';
            $rows.append(html);
        });
    ...
    $(document).on('change', '.variant-size-select', function () {
    ...
    $('form[name="editForm"]').on('submit', function(e) {
        ...
        let rowIndex = 0;
            ...
            $('#variant-wrapper .variant-item').each(function() {
                ...
                // If sizes are selected, create separate entry for each size (size-wise stock & price)
...
// ===== GEMINI AI DESCRIPTION REGENERATOR (Edit) =====
$(document).on('click', '#geminiGenDescBtnEdit', function(){
    var $btn = $(this);
    var productName = $('input[name=\"name\"]').val()?.trim() || $('.page-title').text() || 'product';
    if(!productName){ toastr.error('Product name missing'); return; }
    var categoryText = $('#category_id option:selected').text() || '';
    var lang = $('#geminiDescLangEdit').val() || 'bn';
    $btn.prop('disabled', true).html('<i class=\"fe-loader fe-spin\"></i> Generating...');
    $.ajax({
        url: "{{ route('admin.gemini.generate_description') }}",
        method: "POST",
        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
        data: { product_name: productName, category: categoryText, language: lang, keywords: productName + ' ' + categoryText },
        success: function(res){
            if(res.success && res.message){
                if ($('.summernote').length && typeof $('.summernote').summernote === 'function') {
                    $('.summernote').summernote('code', res.message);
                } else {
                    $('#productDescriptionEdit').val(res.message);
                }
                toastr.success('Description regenerated by Gemini AI');
            } else {
                toastr.error(res.message || 'Generation failed');
            }
        },
        error: function(xhr){
            let msg='Failed'; try{msg=xhr.responseJSON?.message||msg;}catch(e){}
            toastr.error(msg);
        },
        complete: function(){ $btn.prop('disabled', false).html('<i class=\"fe-zap\"></i> ✨ Gemini AI Regenerate'); }
    });
});
</script>
@endsection