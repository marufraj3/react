                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            <label class="form-label d-flex justify-content-between align-items-center">
                                ...
                                <span>
                                    <button type="button" class="btn btn-sm btn-primary rounded-pill px-2" id="geminiGenDescBtn" style="font-size:11px;"><i class="fe-zap"></i> ✨ Gemini AI Generate</button>
                                    <select id="geminiDescLang" style="font-size:11px; padding:2px 6px; border-radius:12px; border:1px solid #dee2e6;"><option value="bn">বাংলা</option><option value="en">English</option></select>
                                </span>
                            </label>
                            <textarea name="description" class="summernote" id="productDescription" required>{{ old('description') }}</textarea>
                            <small class="text-muted">Product name ও category থেকে Gemini AI দিয়ে SEO-friendly description জেনারেট করুন। API Key Settings এ সেট থাকতে হবে।</small>
                        </div>
...
<script>
    ...
    $(document).on('click', '#geminiGenDescBtn', function(){
        var $btn = $(this);
        var productName = $('input[name=\"name\"]').val()?.trim();
        if(!productName){
            toastr.error('Please enter Product Name first');
            $('input[name=\"name\"]').focus();
            return;
        }
        var categoryText = $('#category_id option:selected').text() || '';
        var lang = $('#geminiDescLang').val() || 'bn';
        ...
        $.ajax({
            ...
            data: {
                ...
                language: lang,
                keywords: productName + ' ' + categoryText
            },
            success: function(res){
                if(res.success && res.message){
                    ...
                    if ($('.summernote').length && typeof $('.summernote').summernote === 'function') {
                        ...
                        $('#productDescription').val(res.message);
                    }
                    toastr.success('Description generated by Gemini AI');
                } else {
            ...
            error: function(xhr){
                let msg = 'Failed';
                try { msg = xhr.responseJSON?.message || xhr.responseText.substring(0,200); } catch(e){}
                toastr.error(msg);
                if(xhr.status === 400 && xhr.responseJSON?.message?.includes('API Key')){
                    if(confirm('Gemini API Key not configured. Go to Settings?')){
                        window.open("{{ route('admin.gemini.settings') }}", '_blank');
                    }
                }
            },
            complete: function(){
                $btn.prop('disabled', false).html('<i class=\"fe-zap\"></i> ✨ Gemini AI Generate');
            }
        });
    });
</script>
